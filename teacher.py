...
class Teacher:
    def __init__(self,root):
        ...
        img3 = PIL.Image.open(r"img/bgnt.png")
        ...
        #==================================heading====================================
        #====time====
        img_time = PIL.Image.open(r"img/timsearch50.png")
    ...
    def add_data(self):
        if self.var_securityQ.get()=="Select" or self.var_id.get()=="" or self.var_name.get()=="":
            ...
            try:
                ...
                handler.execute("insert into teacher values(%s,%s,%s,%s,%s,%s,%s)",(
                    self.var_id.get(),
                    self.var_name.get(),
                    self.var_phone.get(),
                    self.var_email.get(),
    ...
    def fetch_data(self):
            ...
            handler.execute("Select * from teacher")
    ...
    def update_data(self):
        if self.var_securityQ.get()=="Select" or self.var_id.get()=="" or self.var_name.get()=="":
            ...
            try:
                ...
                if Update>0:
                    conn, handler = connect_db()
                    handler.execute("update teacher set Name=%s,Phone=%s,Email=%s,SecurityQ=%s,SecurityA=%s,Password=%s"
                                      " where Teacher_id=%s",(
                                            self.var_name.get(),
                                            self.var_phone.get(),
                                            self.var_email.get(),
    ...
    def search_data(self):
        if self.var_com_search.get()=="" or self.var_search.get()=="":
            ...
            try:
                ...
                handler.execute("select * from teacher where "+str(self.var_com_search.get())+" Like '%"+str(self.var_search.get())+"%'")
                data=handler.fetchall()
                if(len(data)!=0):
                    self.AttendanceReportTable.delete(*self.AttendanceReportTable.get_children())
                    for i in data:

...
class Report:
    def __init__(self,root):
        ...
        #========no-attendance=======
        my_db.execute("select count(student.Student_id) from student,lesson,student_has_subject,`subject` where student.Student_id=student_has_subject.Student_id "
    ...
    def fetch_Latedata(self):
            ...
            conn = mysql.connector.connect(host='localhost', user='root', password='', database='pythonsystem',
                                           port='3306')
            my_db = conn.cursor()
            my_db.execute("select DISTINCT Student_id,`Name`,attendance.Date,Subject_name,lesson.Lesson_id,AttendanceStatus from attendance,`subject`,lesson where   AttendanceStatus like '%Đi muộn%' "
    ...
    def search_Latedata(self):
        if self.var_com_searchlate.get()=="" or self.var_searchlate.get()=="":
            ...
            try:
                ...
                mydata.clear()
                my_db.execute("select DISTINCT Student_id,`Name`,attendance.Date,Subject_name,lesson.Lesson_id,AttendanceStatus from attendance,`subject`,lesson where"
    ...
    def fetch_Notdata(self):
            ...
            conn = mysql.connector.connect(host='localhost', user='root', password='', database='pythonsystem',
                                           port='3306')
            my_db = conn.cursor()
            my_db.execute("select DISTINCT Student_id,`Name`,attendance.Date,Subject_name,lesson.Lesson_id,AttendanceStatus from attendance,`subject`,lesson where   AttendanceStatus like '%Vắng%' "
                              "and lesson.Subject_id=subject.Subject_id and attendance.Lesson_id=lesson.Lesson_id")
            data = my_db.fetchall()
            if len(data) != 0:
    ...
    def fetch_Notindata(self):
            ...
            conn = mysql.connector.connect(host='localhost', user='root', password='', database='pythonsystem',
                                           port='3306')
            my_db = conn.cursor()
            my_db.execute("select student.Student_id,Name,Date,Subject_name,Lesson_id from student,lesson,student_has_subject,`subject` "
    ...
    def search_Notindata(self):
        if self.var_com_searchNotin.get()=="" or self.var_searchNotin.get()=="":
            ...
            try:
                ...
                mydataNotInAtt.clear()
                my_db.execute("select student.Student_id,Name,Date,Subject_name,Lesson_id from student,lesson,student_has_subject,`subject` where "

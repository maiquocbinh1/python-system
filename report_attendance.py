...
class Report:
    def __init__(self, root):
        ...
        img3 = PIL.Image.open(r"img/bg1.png")
        ...
        my_db = conn.cursor()
        # ========student==========
        my_db.execute("select count(*) from student")
        count_st = my_db.fetchone()
        self.student.set(count_st[0])
        # =======attendance========
        my_db.execute("select count(*) from attendance")
        count_att = my_db.fetchone()
        self.att.set(count_att[0])
        # =======late=============
        my_db.execute("select  count(Student_id) from attendance where AttendanceStatus like '%Đi muộn%'")
        count_late = my_db.fetchone()
        self.late.set(count_late[0])
        # ========no-attendance=======
        my_db.execute(
            "select count(student.Student_id) from student,lesson,student_has_subject,`subject` where student.Student_id=student_has_subject.Student_id "
        ...
        count_noatt = my_db.fetchone()
        # in_attendance_table
        my_db.execute("select  count(Student_id) from attendance where AttendanceStatus like '%Vắng%'")
        count_noatt1 = my_db.fetchone()
        ...
        img_student = PIL.Image.open(r"img/sv.png")
        ...
        student_text = Label(student_frame, text="Số sinh viên", font=("times new roman", 20, "bold"), fg="white",
                             bg="#27a9e3")
        student_text.place(x=100, y=30)
        student_text = Label(student_frame, textvariable=self.student, font=("times new roman", 16, "bold"), fg="white",
                             bg="#27a9e3")
        ...
        img_att = PIL.Image.open(r"img/sodd.png")
        ...
        img_late = PIL.Image.open(r"img/late.png")
        ...
        img_noatt = PIL.Image.open(r"img/vang.png")
        ...
        # topleft
        late_group = LabelFrame(Left_frame, bd=1, bg="white", text="Sinh viên đi muộn",
        ...
        noatt_lbl = Label(Right_frame, bd=0, bg="white", text="Sinh viên không điểm danh",
    ...
    def fetch_Latedata(self):
        ...
        my_db = conn.cursor()
        my_db.execute(
            "select DISTINCT Student_id,`Name`,attendance.Date,Subject_name,lesson.Lesson_id,AttendanceStatus from attendance,`subject`,lesson where   AttendanceStatus like '%Đi muộn%' "
    ...
    def search_Latedata(self):
        if self.var_com_searchlate.get() == "" or self.var_searchlate.get() == "":
            ...
            try:
                ...
                mydata.clear()
                my_db.execute(
                    "select DISTINCT Student_id,`Name`,attendance.Date,Subject_name,lesson.Lesson_id,AttendanceStatus from attendance,`subject`,lesson where"
    ...
    def exportCsv(self):
        try:
            ...
            with open('Attendance_CSV/diemdanhmuon.csv', mode="w", newline="", encoding="utf-8") as myfile:
                ...
                messagebox.showinfo("Xuất Dữ Liệu", "Dữ liệu của bạn đã được xuất đến " + os.path.basename(
                    'Attendance_CSV/diemdanhmuon.csv') + " thành công", parent=self.root)
        except Exception as es:
            messagebox.showerror("Lỗi", f"Due To:{str(es)}", parent=self.root)
    ...
    def fetch_Notdata(self):
        ...
        my_db = conn.cursor()
        my_db.execute(
            "select DISTINCT Student_id,`Name`,attendance.Date,Subject_name,lesson.Lesson_id,AttendanceStatus from attendance,`subject`,lesson where   AttendanceStatus like '%Vắng%' "
            "and lesson.Subject_id=subject.Subject_id and attendance.Lesson_id=lesson.Lesson_id")
        data = my_db.fetchall()
        if len(data) != 0:
        ...
        try:
            ...
            with open('Attendance_CSV/diemdanhvang.csv', mode="w", newline="", encoding="utf-8") as myfile:
                ...
                for i in mydataNot:
                    exp_write.writerow(i)
                messagebox.showinfo("Xuất Dữ Liệu", "Dữ liệu của bạn đã được xuất đến " + os.path.basename(
                    'Attendance_CSV/diemdanhvang.csv') + " thành công", parent=self.root)
        except Exception as es:
            messagebox.showerror("Lỗi", f"Due To:{str(es)}", parent=self.root)
    ...
    def fetch_Notindata(self):
        ...
        my_db = conn.cursor()
        my_db.execute(
            "select student.Student_id,Name,Date,Subject_name,Lesson_id from student,lesson,student_has_subject,`subject` "
    ...
    def search_Notindata(self):
        if self.var_com_searchNotin.get() == "" or self.var_searchNotin.get() == "":
            ...
            try:
                ...
                mydataNotInAtt.clear()
                my_db.execute(
                    "select student.Student_id,Name,Date,Subject_name,Lesson_id from student,lesson,student_has_subject,`subject` where "
        ...
        try:
            ...
            with open('Attendance_CSV/khongdiemdanh.csv', mode="w", newline="", encoding="utf-8") as myfile:
                ...
                for i in mydataNotInAtt:
                    exp_write.writerow(i)
                messagebox.showinfo("Xuất Dữ Liệu", "Dữ liệu của bạn đã được xuất đến " + os.path.basename(
                    'Attendance_CSV/khongdiemdanh.csv') + " thành công")
        except Exception as es:
            messagebox.showerror("Lỗi", f"Due To:{str(es)}", parent=self.root)

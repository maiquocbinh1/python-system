...
class Subject:
    def __init__(self,root):
        ...
        img3 = PILImage.open(r"img/bgbtn.png")
        img3 = img3.resize((1530, 790), PILImage.LANCZOS)
        ...
        #==================================heading====================================
        #====time====
        img_time = PILImage.open(r"img/timsearch50.png")
        img_time = img_time.resize((27, 27), PILImage.LANCZOS)
        ...
        #====title=========
        self.heading = Label(self.root, text="Quản lý thông tin môn học", font=("yu gothic ui", 32, "bold"), bg="white", fg="black")
        self.heading.place(relx=0.5, y=22, anchor="n")
        ...
        self.AttendanceReportTable.bind("<ButtonRelease>", self.get_row)
        ...
        self.TeacherTable.bind("<ButtonRelease>", self.get_row_teacher)
        self.fetch_Tcdata()
        ...
        self.StudentTable.bind("<ButtonRelease>", self.get_row_student)
        self.fetch_Stddata()
        # ================fetchData======================

    def getNextid(self):
        conn = mysql.connector.connect(host='localhost', user='root', password='', database='pythonsystem',
                                       port='3306')
        db = conn.cursor()
        db.execute(
    ...
    def add_data(self):
        ...
        db = conn.cursor()
        #=========check subject============
        db.execute("select Subject_name from `subject` where Class=%s",(self.var_subclass.get(),))
        ...
        #========check class================
        db.execute("select Class from `class` ")
        ckclass = db.fetchall()
        arrayClass=[]
        for chc in ckclass:
            print(chc[0])
        ...
        #============checkid=================
        db.execute("select Subject_id from `subject` ")
        cksubject = db.fetchall()
        arraySubject = []
        ...
        if self.var_subid.get()=="Select" or self.var_subname.get()=="" or self.var_subclass.get()=="":
            ...
            try:
                ...
                if Update>0:
                    conn=mysql.connector.connect(host='localhost', user='root', password='', database='pythonsystem', port='3306')
                    db = conn.cursor()
                    db.execute("update subject set Subject_name=%s,Class=%s"
                                      " where Subject_id=%s",(
                                            self.var_subname.get(),
                                            self.var_subclass.get(),
                                            self.var_subid.get(),
                ...
                try:
                    ...
                    if delete > 0:
                        conn = mysql.connector.connect(host='localhost', user='root', password='',
                                                       database='pythonsystem', port='3306')
                        db = conn.cursor()
    ...
    def search_data(self):
        if self.var_com_search.get()=="" or self.var_search.get()=="":
            ...
            try:
                conn = mysql.connector.connect(host='localhost', user='root', password='',
                                               database='pythonsystem', port='3306')
                db = conn.cursor()#"ID Điểm Danh", "Ngày", "ID Sinh Viên"
                ...
                db.execute("select * from subject where "+str(self.var_com_search.get())+" Like '%"+str(self.var_search.get())+"%'")
                data=db.fetchall()
                if(len(data)!=0):
                    self.AttendanceReportTable.delete(*self.AttendanceReportTable.get_children())
                    for i in data:
    ...
    def get_row_teacher(self, event=""):
    ...
    def callback(self):
        conn = mysql.connector.connect(host='localhost', user='root', password='', database='pythonsystem',
                                       port='3306')
        db = conn.cursor()
        db.execute("select Teacher_id from `teacher` ")
        ckteacher = db.fetchall()
        arrayTeacher = []
        ...
        if(self.var_teachersub.get() not in arrayTeacher):
            ...
            conn = mysql.connector.connect(host='localhost', user='root', password='', database='pythonsystem',
                                           port='3306')
            db = conn.cursor()
            db.execute("select Name from `teacher` where Teacher_id=%s", (self.var_teachersub.get(),))
            ckteacher = db.fetchone()
    ...
    def callsubtc(self):
        ...
        if(self.var_subsub.get() not in arrayTeacher):
            ...
            conn = mysql.connector.connect(host='localhost', user='root', password='', database='pythonsystem',
                                           port='3306')
            db = conn.cursor()
            db.execute("select Subject_name from `subject` where Subject_id=%s", (self.var_subsub.get(),))
            ckteacher = db.fetchone()
    ...
    def add_Tcdata(self):
        ...
        db = conn.cursor()
        # =========check subject============
        db.execute("select Subject_id from teacher_has_subject where Teacher_id=%s", (self.var_teachersub.get(),))
        ckSubject = db.fetchall()
        arraySub = []
        for chk in ckSubject:
            arraySub.append(str(chk[0]))
        ...
        db.execute("select COUNT(Teacher_id) from `teacher_has_subject` where Subject_id=%s",(self.var_subsub.get(),) )
        ckTeacher_has1sub = db.fetchone()
        ...
        if self.var_teachersub.get()=="" or self.var_subsub.get()=="" :
            ...
            try:
                conn = mysql.connector.connect(host='localhost', user='root', password='', database='pythonsystem', port='3306')
                db=conn.cursor()
                db.execute("insert into teacher_has_subject values(%s,%s)",(
    ...
    def fetch_Tcdata(self):
            ...
            conn = mysql.connector.connect(host='localhost', user='root', password='', database='pythonsystem',
                                           port='3306')
            db = conn.cursor()
            db.execute("Select * from teacher_has_subject")
            data = db.fetchall()
            if len(data) != 0:
    ...
    def update_Tcdata(self):
        ...
        db = conn.cursor()
        # =========check subject============
        db.execute("select Subject_id from teacher_has_subject where Teacher_id=%s",
                          (self.var_teachersub.get(),))
        ckSubject = db.fetchall()
        arraySub = []
        for chk in ckSubject:
        ...
        if self.var_teachersub == "" or self.var_subsub.get() == "":
            ...
            try:
                ...
                if Update>0:
                    conn=mysql.connector.connect(host='localhost', user='root', password='', database='pythonsystem', port='3306')
                    db = conn.cursor()
                    db.execute("UPDATE `teacher_has_subject` SET `Teacher_id` = %s, `Subject_id` = %s WHERE "
    ...
    def search_Tcdata(self):
        if self.var_com_searchtc.get()=="" or self.var_searchtc.get()=="":
            ...
            try:
                ...
                db.execute("select * from teacher_has_subject where "+str(self.var_com_searchtc.get())+" Like '%"+str(self.var_searchtc.get())+"%'")
                data=db.fetchall()
                if(len(data)!=0):
                    self.TeacherTable.delete(*self.TeacherTable.get_children())
                    for i in data:
    ...
    def get_row_student(self, event=""):
    ...
    def callstudent(self):
        conn = mysql.connector.connect(host='localhost', user='root', password='', database='pythonsystem',
                                       port='3306')
        db = conn.cursor()
        db.execute("select Student_id from `student` ")
        ...
        if(self.var_studentsub.get() not in arrayStudent):
            ...
            conn = mysql.connector.connect(host='localhost', user='root', password='', database='pythonsystem',
                                           port='3306')
            db = conn.cursor()
            db.execute("select Name from `student` where Student_id=%s", (self.var_studentsub.get(),))
            ckStudent = db.fetchone()
    ...
    def callstSub(self):
        ...
        if(self.var_subsubst.get() not in arraySubject):
            ...
            conn = mysql.connector.connect(host='localhost', user='root', password='', database='pythonsystem',
                                           port='3306')
            db = conn.cursor()
            db.execute("select Subject_name from `subject` where Subject_id=%s", (self.var_subsubst.get(),))
            cksubject = db.fetchone()
    ...
    def add_Stddata(self):
        ...
        db = conn.cursor()
        # =========check subject============
        db.execute("select Subject_id from student_has_subject where Student_id=%s",
...
>>>>>>> parent of b8eafba (hoan thien not chuc nang thong ke va cap nhat document)
        ...
        if self.var_studentsub == "" or self.var_subsubst.get() == "":
            ...
            try:
                conn = mysql.connector.connect(host='localhost', user='root', password='',
                                                   database='pythonsystem', port='3306')
                db = conn.cursor()
                db.execute("insert into student_has_subject values(%s,%s)", (
                        self.var_studentsub.get(),
    ...
    def fetch_Stddata(self):
            ...
            conn = mysql.connector.connect(host='localhost', user='root', password='', database='pythonsystem',
                                           port='3306')
            db = conn.cursor()
            db.execute("Select * from student_has_subject")
            data = db.fetchall()
            if len(data) != 0:
            ...
            try:
                ...
                if Update > 0:
                    conn = mysql.connector.connect(host='localhost', user='root', password='',
                                                       database='pythonsystem', port='3306')
                    db = conn.cursor()
                    db.execute("UPDATE `student_has_subject` SET `Student_id` = %s, `Subject_id` = %s WHERE "
    ...
    def search_Stddata(self):
            if self.var_com_searchstd.get() == "" or self.var_searchstd.get() == "":
                ...
                try:
                    ...
                    db.execute("select * from student_has_subject where " + str(

...
class Login_Window:
    def __init__(self, root):
        ...
        Checkbutton(login_frame, variable=self.varcheck, text="Đăng nhập bằng tài khoản Admin",
        ...
        Button(login_frame, text="Đăng nhập", command=self.login, font=("times new roman", 17, "bold"), fg="white",
               bg="#B00857", bd=0, cursor="hand2").place(x=250, y=400, width=220, height=40)
    ...
    def clock_image(self, hr, min_, sec_):
        clock = PILImage.new("RGB", (400, 400), (8, 25, 35))
        draw = ImageDraw.Draw(clock)
        clock_path = os.path.join(BASE_DIR, "img", "clock.png")
        bg = PILImage.open(clock_path).resize((300, 300), PILImage.Resampling.LANCZOS)
    ...
    def login(self):
        ...
        if self.varcheck.get() == 1:
            if self.var_email.get() == email_demo and self.var_password.get() == pass_demo:
                new_print("0")
                self.reset()
                messagebox.showinfo("Thông báo", "Đăng nhập demo thành công với quyền Admin")
                self.new_window = Toplevel(self.root)
                self.app = Face_Recognition_System(self.new_window)
            else:
                messagebox.showerror("Lỗi", "Sai tài khoản hoặc mật khẩu (demo)")
        else:
            if self.var_email.get() == email_demo and self.var_password.get() == pass_demo:
                new_print("GV001")
                self.reset()
